...
public class JournalEntryController {
    ...
    public ResponseEntity<JournalEntries> getJournalEntryById(@PathVariable ObjectId myId)
    {
        ...
        String username = authentication.getName();
        User user = userService.findByUserName(username);
        List<JournalEntries> collect = user.getJournalEntries().stream().filter(x -> x.getId().equals(myId)).collect(Collectors.toList());
        if(!collect.isEmpty())
        {
    ...
    @PutMapping("/id/{myId}")
    public ResponseEntity<JournalEntries> updateJournalById
             ...
             @RequestBody JournalEntries newEntry)
    {
        ...
        if(!collect.isEmpty())
        {
            Optional<JournalEntries> journalEntries = journalEntityService.findById(myId);
            if (journalEntries.isPresent())
            {
                JournalEntries old = journalEntries.get();
